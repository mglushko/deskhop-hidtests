 ...
 * HARNESS_HANDLER_LOOKUP. Upstream ce8abb6 fixes it a third way: a 256-entry map of
 * receiver ids per interface, resolved through report_receivers[], which the Makefile
 * names HARNESS_HANDLER_MAP. Every place the harness reads the table goes through
 * hid_handler(), so a change of shape lands here once rather than in five files, and the
 * same tests measure all three. */
#pragma once
